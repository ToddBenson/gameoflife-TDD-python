def get_generation(cells, generation):
    ...
    for counter in range(0, generation):
        ...
        def add_bottom_row(add_bottom_row_array):
            bottom_row = add_bottom_row_array[-1]
            n = [get_active_neighbors(len(bottom_row), x) for x in range(len(bottom_row))]
            if max(n) == 3:
                 add_bottom_row_array.append([set_value(get_active_neighbors((len(add_bottom_row_array) + 1), cell), 0) for cell in range(len(add_bottom_row_array[0]))])
        ...
        def add_columns(add_column_array):
            if sum([x[-1] for x in add_column_array]) >= 3 and counter > 0:
